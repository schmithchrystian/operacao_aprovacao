...
import { listPersistedAuditRecords } from "@/server/audit/log";
import { SENSITIVE_ADMIN_ONLY_ROLES } from "./roles";

/**
 * Visualização do log de auditoria (Fase 17 — só `admin`). `listPersistedAuditRecords`
...
export const listAuditLogForAdmin = withAdminAudit(
  ...
  async (_session, input: ListAuditLogInput): Promise<AuditLogPageDTO> => {
    const all = await listPersistedAuditRecords();
